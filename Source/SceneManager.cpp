...
void SceneManager::DefineObjectMaterials()
{
	...
	OBJECT_MATERIAL metallicMaterial;
	metallicMaterial.ambientColor = glm::vec3(0.3f, 0.3f, 0.3f);
	metallicMaterial.ambientStrength = 0.4f;
	metallicMaterial.diffuseColor = glm::vec3(0.4f, 0.4f, 0.4f);
	metallicMaterial.specularColor = glm::vec3(0.2f, 0.5f, 0.4f);
	metallicMaterial.shininess = 10.0;
	metallicMaterial.tag = "metallic";
	m_objectMaterials.push_back(metallicMaterial);
	...
	woodMaterial.ambientColor = glm::vec3(0.3f, 0.3f, 0.3f);
...
void SceneManager::SetupSceneLights()
{
	...
	m_pShaderManager->setVec3Value("lightSources[0].position", 20.0f, 12.0f, -20.0f);
	...
	m_pShaderManager->setVec3Value("lightSources[1].position", -20.0f, 12.0f, -20.0f);
...
void SceneManager::LoadSceneTextures()
{
	...
	bReturn = CreateGLTexture(
		"../../Utilities/textures/Marble_texture.jpg", "table");
	bReturn = CreateGLTexture(
		"../../Utilities/textures/pencil_blue.jpg",
		"pencil");
	bReturn = CreateGLTexture(
		"../../Utilities/textures/Metallic_texture.jpg",
		"eraser_holder");
	bReturn = CreateGLTexture(
		"../../Utilities/textures/Red.png",
		"red");
	bReturn = CreateGLTexture(
		"../../Utilities/textures/Yellow.png",
		"yellow");
	bReturn = CreateGLTexture(
		"../../Utilities/textures/Blue.png",
		"blue");
	bReturn = CreateGLTexture(
		"../../Utilities/textures/Green.png",
		"green");
	bReturn = CreateGLTexture(
		"../../Utilities/textures/White.png",
		"white");
	bReturn = CreateGLTexture(
		"../../Utilities/textures/Orange.png",
		"orange");
	bReturn = CreateGLTexture(
		"../../Utilities/textures/Glass_texture.jpg",
		"glassy");
...
void SceneManager::RenderScene()
{
	...
	// Give the plane mesh a marble texture similar to the 2D image
	...
	// draw the pencil body using an elongated cylinder
	// set the XYZscale for the cylinder shape
	...
	// draw the pencil head using a cone
	// set the XYZscale for the cone shape
	...
	// set the color values into the shader
	SetShaderColor(0.961, 0.961, 0.863, 1);
	// set object material
	SetShaderMaterial("wood");
	...
	// draw the pencil rear metal piece using cylinder
	// set the XYZscale for the cylinder shape
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(4.5f, 0.5f, 7.35f);
	...
	// give the eraser holder a more metallic texture to fit with the 2D image
	SetShaderTexture("eraser_holder");
	SetTextureUVScale(1.0, 1.0);
	// set object material
	SetShaderMaterial("metallic");
	...
	// draw the pencil erasor using cylinder
	// set the XYZscale for the cylinder shape
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(4.7f, 0.5f, 7.7f);
	...
	// draw the rubik's cube using texture mapping per face
	// set the XYZscale for the box shape
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(5.0f, 1.0f, 1.1f);
	...
	// draw the notebook using a box mesh
	// set the XYZscale for the box shape
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(-2.0f, 0.5f, 6.0f);
	...
	// draw the glass with the tapered cylinder mesh
	// set the XYZscale for the tapered cylinder
	scaleXYZ = glm::vec3(1.5f, 4.0f, 1.0f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(-6.0f, 4.2f, 0.0f);
	...
	// set the color values into the shader
	//SetShaderColor(0.961, 0.961, 0.863, 1);
	// give the glass a glass like texture
	SetShaderTexture("glassy");
	// set object material
	...
	// the following are the metal rings for the notebook. There will be 3 total rings
	// draw the notebook metal rings with the torus mesh
	// set the XYZscale for the torus mesh
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(-4.7f, 0.5f, 4.6f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(-4.35f, 0.5f, 6.5f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(-4.1f, 0.5f, 8.1f);
